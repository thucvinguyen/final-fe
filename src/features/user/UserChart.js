...
function UserChart({ selectedUser }) {
  const [selectedMonth, setSelectedMonth] = useState(0);
  const [selectedYear, setSelectedYear] = useState(dayjs().year());

  // Aggregate calories by date for exercise
  const exerciseCalorieData = selectedUser.exercise.reduce((acc, item) => {
  ...
  const mealCalorieData = selectedUser.meal.reduce((acc, item) => {
    const date = dayjs(item.date).format("MM/DD/YYYY");
    acc[date] = (acc[date] || 0) + item.calories;
    return acc;
  }, {});

  // Combine exercise and meal calorie data
